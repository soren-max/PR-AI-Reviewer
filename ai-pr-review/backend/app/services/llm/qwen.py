...
Uses the DashScope API (OpenAI-compatible).  Configuration is read
from ``settings.QWEN_*``.

.. note::

    This provider is functionally identical to the OpenAI adapter at
    the HTTP level — DashScope exposes an OpenAI-compatible endpoint.
    It exists so that users can switch via a single config key without
...
class QwenService(BaseLLMService):
    """LLM service using Alibaba Cloud's Qwen (DashScope) API.

    Supports qwen-max, qwen-turbo, qwen-plus, etc.
    Configuration is read from ``settings.QWEN_*``.
    """
    ...
    def __init__(
        ...
        self._max_tokens = max_tokens or settings.QWEN_MAX_TOKENS
        self._temperature = temperature if temperature is not None else settings.QWEN_TEMPERATURE
        self._timeout = timeout or settings.QWEN_TIMEOUT
    ...
    async def review_pr(
        ...
        try:
            ...
            logger.exception("Qwen review_pr failed")
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        "Qwen API success: %d in / %d out tokens (attempt %d/%d)",
                    ...
                    logger.warning(
                        "Qwen rate limited, retrying after %.1fs (attempt %d/%d)",
                ...
                last_exception = exc
                logger.warning(
                    "Qwen request timed out (attempt %d/%d)",
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMAPIError(
                    f"Qwen API timed out after {self._max_retries} retries"
                ) from exc

            except httpx.RequestError as exc:
                last_exception = exc
                logger.error(
                    "Qwen request error: %s (attempt %d/%d)",
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMAPIError(
                    f"Qwen API request failed after {self._max_retries} retries: {exc}"
                ) from exc

        raise LLMAPIError(
            f"Qwen API call failed after {self._max_retries} retries"
        ) from last_exception

    async def close(self) -> None:
        await self._client.aclose()

    async def __aenter__(self) -> QwenService:
        return self
