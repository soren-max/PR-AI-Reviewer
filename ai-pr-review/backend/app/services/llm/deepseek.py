...
Implements retry with exponential backoff and token tracking.
"""
...
class DeepSeekService(BaseLLMService):
    """LLM service using DeepSeek's Chat Completions API.

    Supports ``deepseek-chat`` (V4) and other DeepSeek models.
    Configuration is read from ``settings.DEEPSEEK_*``.
    """
    ...
    def __init__(
        ...
        self._api_base = (api_base or settings.DEEPSEEK_API_BASE).rstrip("/")
        self._model = model or settings.DEEPSEEK_MODEL
        self._max_tokens = max_tokens or settings.DEEPSEEK_MAX_TOKENS
        self._temperature = temperature if temperature is not None else settings.DEEPSEEK_TEMPERATURE
        self._timeout = timeout or settings.DEEPSEEK_TIMEOUT
    ...
    async def review_pr(
        ...
        """Run a full code review on a Pull Request diff.

        Builds a structured system + user prompt from the PR data,
        calls the DeepSeek API with retry logic, parses the markdown
        output into sections, and returns a typed result.
        """
        ...
        try:
            ...
            logger.exception("DeepSeek review_pr failed")
        ...
        for attempt in range(1, self._max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        "DeepSeek API success: %d in / %d out tokens (attempt %d/%d)",
                    ...
                    logger.warning(
                        "DeepSeek rate limited, retrying after %.1fs (attempt %d/%d)",
                ...
                last_exception = exc
                logger.warning(
                    "DeepSeek request timed out (attempt %d/%d)",
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMAPIError(
                    f"DeepSeek API timed out after {self._max_retries} retries"
                ) from exc

            except httpx.RequestError as exc:
                last_exception = exc
                logger.error(
                    "DeepSeek request error: %s (attempt %d/%d)",
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMAPIError(
                    f"DeepSeek API request failed after {self._max_retries} retries: {exc}"
                ) from exc

        raise LLMAPIError(
            f"DeepSeek API call failed after {self._max_retries} retries"
        ) from last_exception
    ...
    async def __aenter__(self) -> DeepSeekService:
        return self
...
def _extract_section(markdown: str, section_title: str) -> str:
    """Extract a section by its markdown heading from the LLM output.

    Looks for ``## <section_title>`` or ``### <section_title>`` headings.
    Returns the content between the target heading and the next heading
    of equal or higher level.
    """
    import re

    # Match the heading: ## or ### followed by the title
    pattern = re.compile(
    ...
    start = match.end()
    # Find next heading of level 2 or 3
